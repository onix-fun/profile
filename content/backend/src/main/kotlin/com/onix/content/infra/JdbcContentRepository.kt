        ...
        ds.connection.use { conn ->
            ...
            try {
                conn.prepareStatement(
                    ...
                    SET title = ?, text = ?, allow_comments = ?, visibility = ?, updated_at = ?
                    WHERE id = ?
                    """.trimIndent()
                ).use { ps ->
                    ...
                    ps.setTimestamp(5, Timestamp.from(post.updatedAt))
                    ps.setObject(6, UUID.fromString(post.id))
                    ps.executeUpdate()
                }
        ...
        ds.connection.use { conn ->
            ...
            try {
                conn.prepareStatement(
                    ...
                    ps.setTimestamp(2, Timestamp.from(comment.updatedAt))
                    ps.setObject(3, UUID.fromString(comment.id))
                    ps.executeUpdate()
                }
