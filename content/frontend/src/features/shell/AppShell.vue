...
import { accountSettingsUrl, profileUrl } from "@/api/navigation";
...
const profilePath = computed(() => {
  ...
  if (!owner) return profileUrl("/u/me");
  const prefix = owner.ownerType === "ORGANIZATION" ? "o" : "u";
  return profileUrl(`/${prefix}/${encodeURIComponent(owner.username)}`);
});
    ...
    <Transition name="menu-fade">
      <aside v-if="menuOpen && !controlsHidden" class="account-menu" aria-label="Account menu">
        ...
        <nav class="account-menu__nav">
          ...
          <a :href="profilePath" @click="closeMenu"><i :class="isOrg ? 'pi pi-building' : 'pi pi-user'"></i>Profile</a>
          <a :href="profileUrl('/search')" @click="closeMenu"><i class="pi pi-search"></i>Search</a>
