...
import type { CollectionVisibility, SavedCollection } from "@/api/types";
...
const canCreate = computed(() => title.value.trim().length > 0 && !isSaving.value);
...
async function createCollection() {
  ...
  try {
    ...
    selected.value = new Set([...selected.value, created.id]);
    title.value = "";
    description.value = "";
    ...
    <section class="collection-popover" role="dialog" aria-label="Save to collections">
      ...
      <template v-else>
        ...
        <form class="collection-create" @submit.prevent="createCollection">
          <input v-model="title" maxlength="80" placeholder="New collection" />
          <textarea v-model="description" maxlength="280" rows="2" placeholder="Description"></textarea>
          <span class="collection-create__bar">
            <button
              type="button"
              :class="{ 'is-active': visibility === 'PRIVATE' }"
            ...
            <button
              type="button"
              :class="{ 'is-active': visibility === 'PUBLIC' }"
...
.collection-popover textarea {
  font: inherit;
}

.collection-popover header button,
.collection-create__bar button {
  width: 34px;
  height: 34px;
  border: 0;
  border-radius: 999px;
...
.collection-create__bar button.is-active {
  background: #111827;
  color: #ffffff;
}

.collection-create__bar button:disabled,
