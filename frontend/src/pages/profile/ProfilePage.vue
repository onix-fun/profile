...
import type { AccountProfile, ContentBlock, ProfileCanvasResponse, ProfileContentPost, ProfileNavButton, SavedCollection, SessionUser } from "@/shared/api/types";
import PostCloudNode from "@/features/content/ui/PostCloudNode.vue";
import SaveToCollectionsPopover from "@/features/content/ui/SaveToCollectionsPopover.vue";
import { filterProfileNavigation, postEmbedNavigation, serviceFilterParam, withEmbedQuery } from "@/features/embed/lib/profileEmbed";
import {
...
async function loadProfile() {
  ...
  try {
    currentUser.value = await ProfileService.session().catch(() => null);
...
async function toggleFollow() {
  ...
  if (!currentUser.value) return redirectToAccount();
  try {
...
function handleNavButton(button: ProfileNavButton) {
  ...
  if (button.mode === "canvas" && (button.key === "posts" || button.backendOperation === "posts")) {
    activeMode.value = "posts";
  }
}

function navButtonStyle(button: ProfileNavButton, index: number) {
  const positions = [
    { left: 0, top: 0 },
    { left: 34, top: 62 },
    { left: 0, top: 124 },
  ...
  const position = positions[index] || { left: (index % 2) * 34, top: index * 62 };
  return {
...
function createButtonStyle() {
  const index = navButtons.value.length;
  return {
    ...navButtonStyle({
      key: "create",
      serviceKey: "profile",
...
function isNavActive(button: ProfileNavButton): boolean {
  return (button.key === "collections" && activeMode.value === "collections")
    || (button.key === "posts" && activeMode.value === "posts");
}
        ...
        <div class="canvas-stage" :style="canvasStageStyle">
          ...
          <nav class="profile-mode-nav" :style="modeNavStyle" aria-label="Profile sections">
            <button
              ...
              :key="button.key"
              type="button"
              :class="{ 'is-active': isNavActive(button), 'is-route': button.mode === 'redirect' || button.kind === 'route' || button.kind === 'redirect' }"
              :title="button.label"
