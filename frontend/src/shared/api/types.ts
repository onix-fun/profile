...
export interface SetItemCollectionsInput {
  collectionIds: string[];
}

export interface ProfileNavButton {
  key: string;
  serviceKey: string;
  featureKey: string;
  label: string;
  icon: string;
  color: string;
  mode?: "redirect" | "canvas" | string;
  kind: "collections" | "section" | "route" | "redirect" | string;
  route?: string | null;
...
export interface SearchFacet {
  group: "type" | "provider" | "tag" | "owner" | "dateRange" | string;
  value: string;
  label: string;
  count: number;
  selected: boolean;
...
export interface SearchResponse {
  ...
  partialErrors: string[];
  facets: SearchFacet[];
  providerStatuses: SearchProviderStatus[];
}
